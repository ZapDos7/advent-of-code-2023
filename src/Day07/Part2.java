...
//                new AbstractMap.SimpleEntry<>('J', 10),
                ...
                new AbstractMap.SimpleEntry<>('J', 0)
        );
    ...
    private static class CardInfo {
      ...
      New rule: J can pretend to be whatever card is best for the purpose of determining hand type
      * */
            ...
            int numOfJokers = howManyPerChar.getOrDefault(0, 0);
            howManyPerChar.remove(0); // omit Js to calculate the other combos
            var numOfQuintets = howManyPerChar.values().stream().filter(i -> i.equals(5)).toList().size();
            if (numOfJokers == 5) {
                numOfQuintets = 1; // if JJJJJ, one quintet of Js (cannot have 2 quintets)
            }
            var numOfQuartets = howManyPerChar.values().stream().filter(i -> i.equals(4)).toList().size();
            if (numOfJokers == 4) {
                numOfQuartets = 1; // if JJJJx, one quartet of Js (cannot have 2 quartets)
            }
            var numOfTriplets = howManyPerChar.values().stream().filter(i -> i.equals(3)).toList().size();
            if (numOfJokers == 3) {
                numOfTriplets = 1; // if JJJxy, one triplet of Js (cannot have 2 triplets)
            }
            var numOfPairs = howManyPerChar.values().stream().filter(i -> i.equals(2)).toList().size();
            if (numOfPairs == 1) {
                if (numOfJokers == 2) { // aa JJ c --> aaJJ c --> 1 quartet
                    numOfQuartets = 1;
                } else if (numOfJokers == 1) { // aa J b c --> aaJ b c --> 1 triplet
                    numOfTriplets++;
                }
            } else if (numOfPairs == 0) {
                if (numOfJokers == 2) { // JJ a b c --> Ja Jb c --> 2 pairs
                    numOfPairs = 2;
                } else if (numOfJokers == 1) { // J a b c d --> Ja b c d --> 1 pair
                    numOfPairs = 1;
                }
            } else if (numOfPairs == 2) {
                if (numOfJokers == 1) {  // xx yy J --> xx yyJ --> -1 pair, +1 triplet
                    numOfPairs = 1;
                    numOfTriplets = 1;
                }
            }
            ...
            if ((numOfPairs == 1 && numOfTriplets == 1)) {
                return 5; // full house
            }
            if (numOfTriplets == 1) {
        ...
        public int compare(CardInfo cardInfo) {
            // remains unchanged for part 2 - J is the weakest
